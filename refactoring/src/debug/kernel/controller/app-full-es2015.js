{
  ...
  const gameOver = function (text, self, player) {
  ...
  const moveCallback = function (app, code, event, x, y, pos) {
    const p = Dagaz.Model.stringToPos(pos.name, app.design);
  ...
  const mouseCallback = function (app, code, event, x, y, pos) {
    const p = Dagaz.Model.stringToPos(pos.name, app.design);
    if (_.isUndefined(app.list)) return true;
    app.move = app.findMove(app.list.setPosition(p));
    const targets = _.map(app.list.getTargets(), function (p) {
  ...
  const idle = function (app) {
    const ai = app.getAI();
    if (ai !== null) {
      const ctx = app.getContext(app.board.player);
      if (ctx !== null) {
        ...
        const result = ai.getMove(ctx);
        canvas.style.cursor = "default";
        if (result && result.done) {
          let player = app.design.playerNames[app.board.player];
  ...
  const wait = function (app) {
    return false;
  };

  const stop = function (app) {
    return true;
  };
  ...
  class App {
    ...
    findMove(move) {
      ...
      for (let i = 0; i < this.board.moves.length; i++) {
    ...
    getContext(player) {
      ...
      delete this.move;
      const g = this.board.checkGoals(this.design, this.board.parent.player);
      if (g !== null) {
        const player = this.design.playerNames[this.board.parent.player];
