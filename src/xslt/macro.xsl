      ...
      <xsl:when test="name()='n'">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$name=/r/n[*[position()=1 and name()='a']='macro']/*[position()=2 and name()='n']/*[position()=1 and name()='a']">
            <xsl:variable name="v" select="*[position()&gt;1]"/>
            <xsl:variable name="p" select="/r/n[*[position()=1 and name()='a']='macro' and *[position()=2 and name()='n']/a[position()=1]=$name]/*[position()=2]/*[position()&gt;1]"/>
            <xsl:for-each select="/r/n[*[position()=1 and name()='a']='macro' and *[position()=2 and name()='n']/a[position()=1]=$name]/*[position()&gt;2]">
              <xsl:call-template name="apply_n">
                ...
                <xsl:with-param name="params" select="$p"/>
              </xsl:call-template>
            </xsl:for-each>
          </xsl:when>
          <xsl:otherwise>
