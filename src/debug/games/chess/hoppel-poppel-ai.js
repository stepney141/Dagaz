...
var materialTable = [0, 800, 4000, 3500, 5000, 9750, 600000];
...
function Mobility(color) {
    ...
    while (from != 0) {
        x = g_board[from + 31] & 0x7; if (x == 0) mob++;
        x = g_board[from + 33] & 0x7; if (x == 0) mob++;
        x = g_board[from + 14] & 0x7; if (x == 0) mob++;
        x = g_board[from - 14] & 0x7; if (x == 0) mob++;
        x = g_board[from - 31] & 0x7; if (x == 0) mob++;
        x = g_board[from - 33] & 0x7; if (x == 0) mob++;
        x = g_board[from + 18] & 0x7; if (x == 0) mob++;
        x = g_board[from - 18] & 0x7; if (x == 0) mob++;

        to = from - 15; while (g_board[to] == 0) { to -= 15; }
        if (g_board[to] & enemy) {
          mob++;
        ...
        to = from - 17; while (g_board[to] == 0) { to -= 17; }
        if (g_board[to] & enemy) {
          mob++;
          if (!(g_board[to] & piecePawn)) {
    ...
    from = g_pieceList[pieceIdx++];
    while (from != 0) {
        x = g_board[from + 31]; if (x & 0x7) mob += mobUnit[x];
        x = g_board[from + 33]; if (x & 0x7) mob += mobUnit[x];
        x = g_board[from + 14]; if (x & 0x7) mob += mobUnit[x];
        x = g_board[from - 14]; if (x & 0x7) mob += mobUnit[x];
        x = g_board[from - 31]; if (x & 0x7) mob += mobUnit[x];
        x = g_board[from - 33]; if (x & 0x7) mob += mobUnit[x];
        x = g_board[from + 18]; if (x & 0x7) mob += mobUnit[x];
        x = g_board[from - 18]; if (x & 0x7) mob += mobUnit[x];
...
function Evaluate() {
    ...
    if (g_pieceList[pieceQueen << 4] == 0) {
        var kingPos = g_pieceList[(colorWhite | pieceKing) << 4];
        if (kingPos != 0) {
    ...
    if (g_pieceList[(colorWhite | pieceQueen) << 4] == 0) {
        var kingPos = flipTable[g_pieceList[pieceKing << 4]];
        if (kingPos != 0) {
            evalAdjust += pieceSquareAdj[pieceKing][kingPos];
        }
    }
...
var g_bishopDeltas = [31, 33, 14, -14, -31, -33, 18, -18];
var g_knightDeltas = [-15, -17, 15, 17];
...
function See(move) {
    ...
    SeeAddSliderAttacks(to, them, themAttacks, pieceKnight);
    ...
    // Slider attacks
    g_board[from] = 0;
    if (SeeAddKnightAttacks(to, them, themAttacks)) {
        if (captureDeficit > g_seeValues[pieceBishop]) {
    ...
    SeeAddSliderAttacks(to, us, usAttacks, pieceKnight);
