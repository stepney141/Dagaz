...
View2D.prototype.addPhase = function(ix, from, to, piece, phase, steps) {
  if ((from < this.pos.length) && (to < this.pos.length)) {
      this.changes.push({
          phase: phase,
          steps: steps,
          from:  from,
...
View2D.prototype.animate = function() {
  ...
  _.chain(this.changes)
   ...
   .filter(isDone)
   .each(function(frame) {
        if (frame.to >= this.pos.length) return;
...
View2D.prototype.draw = function(canvas) {
  ...
  if (this.allResLoaded() && !isValid) {
      ...
      _.chain(_.range(this.setup.length))
       ...
       .each(function(p) {
           if (p.pos >= this.pos.length) return;
           var x = p.x; var y = p.y;
