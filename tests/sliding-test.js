...
QUnit.test( "Bruteforce AI", function( assert ) {
  Dagaz.Model.InitGame();
  var design = Dagaz.Model.getDesign();
  var board  = Dagaz.Model.getInitBoard();
  design.checkVersion("sliding-puzzle", "distinct")

  var ai = Dagaz.AI.findBot("solver", {}, null);
  assert.ok( ai !== null, "Bruteforce AI found");
  var ctx = Dagaz.AI.createContext(design);
  ai.setContext(ctx, board);
  assert.ok( ctx.board === board, "Context setting");
